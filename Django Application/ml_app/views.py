...
def predictf(model,img,path = './', video_file_name=""):
...
def predict(model, img, path='./', video_file_name=""):
    with torch.no_grad():  # Ensure no gradients are computed
        ...
        img = im_convert(img[:, -1, :, :, :], video_file_name)
        params = list(model.parameters())
        weight_softmax = model.linear1.weight.detach().numpy()
        logits = sm(logits)
        _, prediction = torch.max(logits, 1)
        confidence = logits[:, int(prediction.item())].item() * 100
...
def predict_page(request):
    if request.method == "GET":
        ...
        model = Model(2)
        model_name = "model_93_acc_100_frames_celeb_FF_data.pt"
        #models_location = os.path.join(settings.PROJECT_DIR,'models')
        path_to_model ="C:\\Users\\megha\\OneDrive\\Desktop\\DeepFake-Video-Detection-master\\DeepFake-Video-Detection-master\\Django Application\\models\\model_93_acc_100_frames_celeb_FF_data.pt"
        ...
        try:
            ...
            return render(request, predict_template_name, {'preprocessed_images': preprocessed_images, 'heatmap_images': heatmap_images, "faces_cropped_images": faces_cropped_images, "original_video": video_file_name, "models_location": "models_name", "output": output, "confidence": confidence})
        except:
            return render(request, predict_template_name, {'preprocessed_images': preprocessed_images, 'heatmap_images': heatmap_images, "faces_cropped_images": faces_cropped_images, "original_video": video_file_name, "models_location": "models_name", "output": output, "confidence": confidence})
def about(request):
...
def RegisterationView(request):
    if request.method == 'POST':
        ...
        RegisterModel(name = name,email = email,phone = phone,password = password).save()
        return redirect('ml_app:home')
    else:
...
def LoginView(request):         
    if request.method == 'POST':
        ...
        password = request.POST.get('password')
        try:
            usr = RegisterModel.objects.get(email=email)
...
def ProfileView(request):
    if 'email' in request.session:
        ...
        try:
            ur = RegisterModel.objects.get(email=email)
            if request.method == 'POST':
                name = request.POST.get('name')
                email = request.POST.get('email')
                phone = request.POST.get('phone')
                ...
                return render(request, 'profile.html', {'ur': ur})  
        except RegisterModel.DoesNotExist:
            return redirect('ml_app:register')
    else:
